      ...
      @if ($cartItem->count() > 0)
        <div class="card-body">
          ...
          @foreach ($cartItem as $item)
            <div
              class="row product_data border-bottom border-light my-4 d-flex justify-content-around align-items-center ">
              <div class="col-md-2">
                <img src="{{ asset('upload/product/' . $item->products->image) }}" height="80px" width="90px"
                  alt="Image Here">
              </div>
              ...
              <form class="col-md-2" method="POST" action="{{ url('delete-order', $item->id) }}">
                @csrf
                <button type="submit" class="btn btn-outline-danger deleteCartItem"><i class="fa fa-trash"></i>
                  Delete</button>
              </form>
            </div>
          @endforeach
